...
import NotSearchableLink from './../NotSearchableLink/NotSearchableLink.js';
...
import styles from './EducationListPage.less';

class EducationListPage extends React.Component {
  ...
  render () {
    ...
    const hasNotSearchableItems = this.state.educations.filter((nsList) => {
      nsList = nsList.filter((item) => {
        const status = item.getIn(['acf', 'status']);
    ...
    return (
      <div className={styles.base}>
        ...
        {hasNotSearchableItems ? <NotSearchableLink /> : false}
      </div>
    );
  }
}

EducationListPage.propTypes = {
